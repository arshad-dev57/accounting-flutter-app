...
class AccountingReportSummary {
  ...
  factory AccountingReportSummary.fromJson(Map<String, dynamic>? json) {
    if (json == null) {
    ...
    return AccountingReportSummary(
      ...
      debitTotal: AccountingReportRow._toDouble(json['debitTotal']),
      creditTotal: AccountingReportRow._toDouble(json['creditTotal']),
      grandTotal: AccountingReportRow._toDouble(json['grandTotal']),
...
class AccountingReportController extends GetxController {
  ...
  final RxList<AccountingReportRow> rows = <AccountingReportRow>[].obs;
  final Rxn<AccountingReportSummary> summary = Rxn<AccountingReportSummary>();
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        final list = (data['rows'] as List? ?? [])
            .map(
              (e) => AccountingReportRow.fromJson(Map<String, dynamic>.from(e)),
            )
            .toList();
        rows.assignAll(list);
        summary.value = AccountingReportSummary.fromJson(
          data['summary'] is Map
        ...
        error.value = response.message.isNotEmpty
            ? response.message
            : 'Failed to load accounting report';
        rows.clear();
        summary.value = null;
      }
    } catch (e) {
  ...
  Future<List<AccountingReportRow>> _fetchAllForExport() async {
    final response = await _apiClient.get(
      '/api/accounting/reports',
    ...
    return (data['rows'] as List? ?? [])
        .map((e) => AccountingReportRow.fromJson(Map<String, dynamic>.from(e)))
        .toList();
  }
  ...
  Future<void> exportToPdf() async {
    ...
    try {
      ...
      final fileName =
          'accounting_report_${channel.value}_${period.value}_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
      ...
      if (Get.isDialogOpen ?? false) Get.back();
      AppSnackbar.success(kSuccess, 'Success', 'Accounting report PDF downloaded');
    } catch (e) {
      if (Get.isDialogOpen ?? false) Get.back();
  ...
  Future<Uint8List> _generatePdfBytes(List<AccountingReportRow> exportRows) async {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => branding.buildHeader(reportTitle: 'Accounting Report'),
        ...
        build: (context) => [
          ...
          if (s != null) ...[
            ...
            pw.Text(
              'Debit: ${formatCurrency(s.debitTotal)}   Credit: ${formatCurrency(s.creditTotal)}   Posted: ${s.postedCount}   Draft: ${s.draftCount}',
              style: pw.TextStyle(
                fontSize: 9,
          ...
          pw.Container(
            ...
            child: pw.Row(
              children: [
                _pdfCell('Date', flex: 11, bold: true, light: true),
                _pdfCell('Channel', flex: 10, bold: true, light: true),
                _pdfCell('Reference', flex: 13, bold: true, light: true),
                _pdfCell('Description', flex: 18, bold: true, light: true),
                _pdfCell('Status', flex: 10, bold: true, light: true),
                _pdfCell('Debit', flex: 12, bold: true, light: true, right: true),
                _pdfCell('Credit', flex: 12, bold: true, light: true, right: true),
              ],
            ),
          ),
          ...exportRows.map((r) {
            return pw.Container(
              ...
              child: pw.Row(
                children: [
                  _pdfCell(formatDate(r.date), flex: 11),
                  _pdfCell(r.channel.toUpperCase(), flex: 10),
                  _pdfCell(r.reference, flex: 13),
                  _pdfCell(
                    r.description.isNotEmpty ? r.description : r.partyName,
                    flex: 18,
                  ),
                  _pdfCell(r.status, flex: 10),
                  _pdfCell(formatCurrency(r.debit), flex: 12, right: true, bold: true),
                  _pdfCell(formatCurrency(r.credit), flex: 12, right: true, bold: true),
                ],
              ),
            );
          }),
