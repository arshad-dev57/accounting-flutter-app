...
class BalanceSheetController extends GetxController {
  ...
  bool get hasEquitySection =>
      equityData.isNotEmpty || equity.value.abs() >= 0.01;
      ...
      if (response.success) {
        ...
        if (responseData['success'] == true) {
                ...
                for (final item in items) {
                  if (item is Map) {
                    ...
                    if (name.isNotEmpty && balance != 0) {
                      categoryItems[name] = balance;
                    }
                  }
                }
                if (categoryItems.isNotEmpty) {
                  assetsData[_getAssetCategoryName(category.toString())] =
                      categoryItems;
                }
              }
            });
          ...
          if (data['liabilities'] != null && data['liabilities'] is Map) {
            ...
            liabilitiesMap.forEach((category, items) {
              if (items is List && items.isNotEmpty) {
                ...
                if (categoryItems.isNotEmpty) {
                  liabilitiesData[_getLiabilityCategoryName(
                        category.toString(),
                      )] =
                      categoryItems;
                }
          ...
          if (data['equity'] != null && data['equity'] is Map) {
            ...
            final ownersItems = <String, double>{};

            if (equityMap['owners'] != null && equityMap['owners'] is List) {
              for (final item in equityMap['owners'] as List) {
                if (item is Map) {
                  final name = item['name']?.toString() ?? '';
                  final balance = (item['balance'] as num?)?.toDouble() ?? 0;
                  if (name.isNotEmpty && balance != 0) {
                    ownersItems[name] = balance;
                  }
                }
              }
            }

            // Only add retainedEarnings field if not already present in owners list
            final retainedEarnings =
                (equityMap['retainedEarnings'] as num?)?.toDouble() ?? 0;
            if (retainedEarnings != 0) {
              final alreadyListed = ownersItems.keys.any(
                (name) =>
                    name.toLowerCase().contains('current year earnings') ||
                    name.toLowerCase().contains('retained earnings'),
              );
              if (!alreadyListed) {
                ownersItems['Current Year Earnings'] = retainedEarnings;
              }
            }
          ...
          if (data['totals'] != null && data['totals'] is Map) {
            ...
            totalAssets.value =
                (totals['totalAssets'] as num?)?.toDouble() ?? 0;
            totalLiabilities.value =
                (totals['totalLiabilities'] as num?)?.toDouble() ?? 0;
            equity.value = (totals['totalEquity'] as num?)?.toDouble() ?? 0;
          }

          if (data['isBalanced'] != null) {
            isBalanced.value = data['isBalanced'] == true;
            balanceDifference.value =
                (data['difference'] as num?)?.toDouble().abs() ?? 0;
          } else {
            final diff =
                (totalAssets.value - (totalLiabilities.value + equity.value))
                    .abs();
  ...
  String _getAssetCategoryName(String category) {
    switch (category) {
      case 'current':
  ...
  String _getLiabilityCategoryName(String category) {
    switch (category) {
      case 'current':
  ...
  Future<void> exportToPdf() async {
    try {
      ...
      final fileName =
          'balance_sheet_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
      final file = File('${dir.path}/$fileName');
      await file.writeAsBytes(await pdf.save());

      if (Get.isDialogOpen ?? false) Get.back();

      AppSnackbar.success(kSuccess, 'Success', 'Balance sheet exported to PDF');
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              _pdfSummaryItem(
                'Total Assets',
                _formatAmount(totalAssets.value),
                PdfColors.green700,
              ),
              _pdfSummaryItem(
                'Total Liabilities',
  ...
  pw.Widget _pdfLiabilitiesSection() {
    ...
    return pw.Column(
      ...
      children: [
        ...
        pw.SizedBox(height: 8),
        ...liabilitiesData.keys
            .map(
              (category) => _pdfCategorySection(
                category,
                liabilitiesData[category] ?? {},
                true,
              ),
            )
            .toList(),
        ...
        pw.Container(
          ...
          child: pw.Row(
            children: [
              ...
              pw.Text(
                _formatAmount(totalLiabilities.value + equity.value),
                style: pw.TextStyle(
                  fontSize: 12,
                  fontWeight: pw.FontWeight.bold,
                  color: PdfColors.indigo800,
        ...
        pw.Padding(
          ...
          child: pw.Row(
            children: [
              ...
              pw.Text(
                _formatAmount(items.values.fold(0.0, (sum, val) => sum + val)),
                style: pw.TextStyle(
                  fontSize: 10,
                  fontWeight: pw.FontWeight.bold,
                ),
  ...
  Future<void> exportToExcelFile() async {
    try {
      ...
      for (final category in assetsData.keys) {
        ...
        if (items.isNotEmpty) {
          ...
          _excelSetCell(
            ...
            row,
            1,
            items.values.fold(0.0, (sum, val) => sum + val),
            bold: true,
            bgColor: 'E8EAF6',
            fontColor: '2E7D32',
          );
      ...
      _excelSetCell(
        ...
        4,
        1,
        totalLiabilities.value + equity.value,
        bold: true,
        bgColor: 'E8EAF6',
        fontColor: '1A237E',
      );
      ...
      final dir = await getTemporaryDirectory();
      final fileName =
          'balance_sheet_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.xlsx';
      final file = File('${dir.path}/$fileName');
