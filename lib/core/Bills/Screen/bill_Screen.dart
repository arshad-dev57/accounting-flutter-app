...
class BillsScreen extends StatelessWidget {
  ...
  const BillsScreen({super.key, this.supplierId});
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (supplierId != null && supplierId!.isNotEmpty && controller.suppliers.isNotEmpty) {  // ✅ FIXED
        final exists = controller.suppliers.any((s) => s['_id'].toString() == supplierId);  // ✅ FIXED
        controller.filterBySupplier(exists ? supplierId! : '');  // ✅ FIXED
      }
    });
  ...
  Widget _buildMobileLayout(BuildContext context, BillController controller) {
    return Scaffold(
      backgroundColor: kBg,
  ...
  PreferredSizeWidget _buildMobileAppBar(BuildContext context, BillController controller) {
    return AppBar(
      title: const Text(
        'Bills',
  ...
  Widget _buildMobileFilterBar(BillController controller, BuildContext context) {
          ...
          children: statuses.map((s) {
            ...
            return Padding(
              ...
              child: FilterChip(
                label: Text(s),
                selected: isSelected,
                onSelected: (_) => controller.changeFilter(isSelected ? 'All' : s),
                backgroundColor: kBg,
  ...
  Widget _buildMobileSummaryCards(BillController controller, BuildContext context) {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Obx(() => Row(
          children: [
            _buildMobileSummaryCard('Total Amount', _formatAmount(controller.totalAmount.value), kPrimary, Icons.receipt_long),
            const SizedBox(width: 12),
            _buildMobileSummaryCard('Paid', _formatAmount(controller.totalPaid.value), kSuccess, Icons.check_circle_outline),
            const SizedBox(width: 12),
            _buildMobileSummaryCard('Outstanding', _formatAmount(controller.totalOutstanding.value), kDanger, Icons.payment),
            const SizedBox(width: 12),
            _buildMobileSummaryCard('Bills', controller.bills.length.toString(), kPrimary, Icons.receipt, isNumber: true),
          ],
        )),
      ),
    );
  ...
  Widget _buildMobileBillsList(BillController controller, BuildContext context) {
    return Obx(() {
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) => Padding(
          ...
          child: _buildMobileBillCard(bills[index], controller, context),
        ),
      );
    });
  }

  Widget _buildMobileBillCard(Bill bill, BillController controller, BuildContext context) {
  ...
  Widget _buildWebLayout(BuildContext context, BillController controller) {
    return Scaffold(
      backgroundColor: kBg,
      body: Column(
        children: [
  ...
  Widget _buildWebTopBar(BuildContext context, BillController controller) {
    return Container(
      height: 56,
      color: kPrimary,
  ...
  Widget _buildWebKpiStrip(BillController controller) {
    return Obx(() => Container(
      color: kCardBg,
      padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 0),
      child: Row(
        children: [
          _buildWebKpiTile('Total Amount', _formatAmount(controller.totalAmount.value), kPrimary, Icons.receipt_long),
          _buildWebKpiDivider(),
          _buildWebKpiTile('Paid', _formatAmount(controller.totalPaid.value), kSuccess, Icons.check_circle_outline),
          _buildWebKpiDivider(),
          _buildWebKpiTile('Outstanding', _formatAmount(controller.totalOutstanding.value), kDanger, Icons.payment),
          _buildWebKpiDivider(),
          _buildWebKpiTile('Total Bills', controller.bills.length.toString(), kPrimary, Icons.receipt),
        ],
      ),
    ));
  }
  ...
  Widget _buildWebToolbar(BillController controller, BuildContext context) {
    final statuses = ['All', 'Unpaid', 'Paid', 'Overdue', 'Partial'];
    return Container(
      height: 44,
      padding: const EdgeInsets.symmetric(horizontal: 24),
      ...
      child: Row(
        children: [
            ...
            child: Row(
              ...
              children: statuses.map((s) {
                ...
                return Padding(
                  ...
                  child: InkWell(
                    onTap: () => controller.changeFilter(s),
          ...
          Obx(() {
            String? currentValue = controller.selectedSupplierId.value.isEmpty
                ? null
                : controller.selectedSupplierId.value;
            if (currentValue != null) {
              final exists = controller.suppliers.any((s) => s['_id'] == currentValue);
                ...
                child: DropdownButton<String>(
                  ...
                  items: [
                    DropdownMenuItem<String>(
                      value: '',
                      child: Text('All Suppliers', style: TextStyle(fontSize: 12, color: kText)),
                    ),
                  ...
                  onChanged: (v) {
                    if (v != null) controller.filterBySupplier(v);  // ✅ FIXED
                  },
                ),
              ),
            );
  ...
  Widget _buildWebBillsTable(BillController controller, BuildContext context) {
    return Obx(() {
      final bills = controller.bills;
      ...
      return Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(flex: 3, child: _tableHeaderCell('Supplier')),  // ✅ FIXED
          ...
          if (bills.isNotEmpty) _buildWebTableFooter(bills),
        ],
      );
    });
  }
  ...
  Widget _buildWebTableRow(Bill bill, BillController controller, BuildContext context) {
    final statusColor = bill.status == 'Paid'
        ? kSuccess
        : bill.status == 'Overdue'
            ? kDanger
            : bill.status == 'Partial'
                ? kWarning
                : kPrimary;
    final isOverdue = bill.dueDate.isBefore(DateTime.now()) && bill.status != 'Paid';
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                flex: 3,
                child: Text(bill.supplierName,  // ✅ FIXED
                  ...
                  child: Container(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        color: bill.outstanding > 0 ? kDanger : kSuccess,
                      ),
                    ),
                  ),
                ),
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 4),
                    if (bill.status != 'Paid')
                      _webIconBtn(Icons.payment, kSuccess,
                          () => _recordBillPayment(bill, controller, context))
                    else
                      _webIconBtn(Icons.more_vert, kSubText,
                          () => _showRowActions(controller, bill, context)),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildWebTableFooter(List<Bill> bills) {
  ...
  Widget _buildWebPaginationBar(BillController controller) {
    return Obx(() => Container(
      height: 56,
      padding: const EdgeInsets.symmetric(horizontal: 24),
      decoration: BoxDecoration(
  ...
  void _showAddBillDialog(BillController controller, BuildContext ctx) {
    final isWeb = ResponsiveUtils.isWeb(ctx);
    final formKey = GlobalKey<FormState>();
    String selectedSupplierId = '';  // ✅ FIXED
    DateTime selectedDate = DateTime.now();
    DateTime selectedDueDate = DateTime.now().add(const Duration(days: 30));
    List<Map<String, dynamic>> items = [{'description': '', 'quantity': 1, 'unitPrice': 0.0, 'taxRate': 0.0}];
    double discount = 0;
    String notes = '';
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          double calculateTotal() {
            final subtotal = items.fold(0.0, (s, i) => s + (i['quantity'] ?? 1) * (i['unitPrice'] ?? 0.0));
                      ...
                      child: Form(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (isWeb)
                              Row(children: [
                                Expanded(child: _dateTile('Bill Date', selectedDate, (d) => setState(() => selectedDate = d), isWeb, context)),
                                const SizedBox(width: 12),
                                Expanded(child: _dateTile('Due Date', selectedDueDate, (d) => setState(() => selectedDueDate = d), isWeb, context)),
                              ])
                            else ...[
                              _dateTile('Bill Date', selectedDate, (d) => setState(() => selectedDate = d), isWeb, context),
                              const SizedBox(height: 12),
                              _dateTile('Due Date', selectedDueDate, (d) => setState(() => selectedDueDate = d), isWeb, context),
                            ],
                            const SizedBox(height: 12),
                            // Items header
                            Row(
                              ...
                              children: [
                                ...
                                TextButton.icon(
                                  onPressed: () => setState(() => items.add({'description': '', 'quantity': 1, 'unitPrice': 0.0, 'taxRate': 0.0})),
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Expanded(
                                          flex: 2,
                                          child: TextFormField(
                                            decoration: _inputDecoration('Unit Price', isWeb, prefix: CurrencyUtils.prefix),
                                            keyboardType: TextInputType.number,
                                            style: TextStyle(fontSize: isWeb ? 13 : 12, color: kText),
                                            onChanged: (v) { item['unitPrice'] = double.tryParse(v) ?? 0; setState(() {}); },
                                          ),
                                        ),
                                      ],
                                    ),
                            ...
                            const SizedBox(height: 8),
                            TextFormField(
                              decoration: _inputDecoration('Discount', isWeb, prefix: CurrencyUtils.prefix),
                              keyboardType: TextInputType.number,
                              style: TextStyle(fontSize: isWeb ? 13 : 12, color: kText),
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text(_formatAmount(calculateTotal()),
                                      style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15, color: kDanger)),
                                ],
                              ),
                            ),
                          ],
                              ...
                              : () async {
                                  if (formKey.currentState!.validate() && selectedSupplierId.isNotEmpty) {
                                    ...
                                    await controller.createBill({
                                      'supplierId': selectedSupplierId,  // ✅ FIXED
  ...
  void _recordBillPayment(Bill bill, BillController controller, BuildContext ctx) {
    final isWeb = ResponsiveUtils.isWeb(ctx);
    final formKey = GlobalKey<FormState>();
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildDetailRow('Supplier', bill.supplierName, isWeb),  // ✅ FIXED
                    ...
                    TextFormField(
                      ...
                      decoration: _inputDecoration('Payment Amount *', isWeb, prefix: CurrencyUtils.prefix),
                      keyboardType: TextInputType.number,
                      style: TextStyle(fontSize: isWeb ? 13 : 12, color: kText),
                    ...
                    const SizedBox(height: 12),
                    _dateTile('Payment Date', paymentDate, (d) => setState(() => paymentDate = d), isWeb, context),
                    const SizedBox(height: 12),
                    DropdownButtonFormField<String>(
                      value: paymentMethod,
                  ...
                  : () {
                      if (formKey.currentState!.validate()) {
                        ...
                        controller.recordPayment(
                          supplierId: bill.supplierId,  // ✅ FIXED
                          billId: bill.id,
                          amount: amount,
  ...
  void _showBillDetails(Bill bill, BillController controller, BuildContext context) {
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('${bill.supplierName} • ${DateFormat('dd MMM yyyy').format(bill.date)}',  // ✅ FIXED
                            ...
                            children: [
                              Expanded(
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  ...
                                  Text('${item.quantity} × ${_formatAmount(item.unitPrice)}',
  ...
  void _showFilterDialog(BillController controller, BuildContext context) {
    showDialog(
      context: context,
        ...
        content: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () { controller.clearFilters(); Navigator.pop(context); },
            ),
          ],
        ),
        actions: [
  ...
  void _showRowActions(BillController controller, Bill bill, BuildContext context) {
    showMenu(
      context: context,
