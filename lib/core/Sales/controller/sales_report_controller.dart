...
class SalesReportRow {
  ...
  factory SalesReportRow.fromJson(Map<String, dynamic> json) {
    ...
    return SalesReportRow(
      ...
      date: parsed,
      customerName: json['customerName']?.toString() ?? 'Walk-in',
...
class SalesReportSummary {
  ...
  factory SalesReportSummary.fromJson(Map<String, dynamic>? json) {
    ...
    return SalesReportSummary(
      ...
      subtotal: SalesReportRow._toDouble(json['subtotal']),
      taxTotal: SalesReportRow._toDouble(json['taxTotal']),
      discountTotal: SalesReportRow._toDouble(json['discountTotal']),
      grandTotal: SalesReportRow._toDouble(json['grandTotal']),
      byChannel: by,
    );
  }
}
...
class SalesReportController extends GetxController {
  ...
  final RxList<SalesReportRow> rows = <SalesReportRow>[].obs;
  final Rxn<SalesReportSummary> summary = Rxn<SalesReportSummary>();
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        final list = (data['rows'] as List? ?? [])
            .map((e) => SalesReportRow.fromJson(Map<String, dynamic>.from(e)))
            .toList();
        rows.assignAll(list);
        summary.value = SalesReportSummary.fromJson(
          data['summary'] is Map
  ...
  Future<List<SalesReportRow>> _fetchAllForExport() async {
    final response = await _apiClient.get(
      '/api/warehouse/sales/reports',
  ...
  Future<void> exportToPdf() async {
    ...
    try {
      ...
      final fileName =
          'sales_report_${channel.value}_${period.value}_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
      ...
      if (Get.isDialogOpen ?? false) Get.back();
      AppSnackbar.success(kSuccess, 'Success', 'Sales report PDF downloaded');
    } catch (e) {
      if (Get.isDialogOpen ?? false) Get.back();
  ...
  Future<Uint8List> _generatePdfBytes(List<SalesReportRow> exportRows) async {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => branding.buildHeader(reportTitle: 'Sales Report'),
          ...
          pw.Container(
            ...
            child: pw.Row(
              children: [
                ...
                _pdfCell('Customer', flex: 16, bold: true, light: true),
            ...
            return pw.Container(
              ...
              child: pw.Row(
                children: [
                  ...
                  _pdfCell(r.customerName, flex: 16),
