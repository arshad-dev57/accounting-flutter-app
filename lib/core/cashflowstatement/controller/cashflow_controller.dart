...
class CashFlowController extends GetxController {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (responseData['success'] == true) {
          ...
          periodText.value = period['displayText']?.toString() ?? '';

          final operating =
              data['operatingActivities'] as Map<String, dynamic>? ?? {};
          operatingItems.value = ((operating['items'] as List?) ?? [])
              .whereType<Map>()
              .map(
                (item) => CashFlowItem(
                  name: item['name']?.toString() ?? '',
                  amount: _toDouble(item['amount']),
                  type: item['type']?.toString() ?? 'operating',
                ),
              )
              .toList();
          cashFlowFromOperations.value = _toDouble(operating['total']);

          final investing =
              data['investingActivities'] as Map<String, dynamic>? ?? {};
          investingItems.value = ((investing['items'] as List?) ?? [])
              .whereType<Map>()
              .map(
                (item) => CashFlowItem(
                  name: item['name']?.toString() ?? '',
                  amount: _toDouble(item['amount']),
                  type: item['type']?.toString() ?? 'investing',
                ),
              )
              .toList();
          cashFlowFromInvesting.value = _toDouble(investing['total']);

          final financing =
              data['financingActivities'] as Map<String, dynamic>? ?? {};
          financingItems.value = ((financing['items'] as List?) ?? [])
              .whereType<Map>()
              .map(
                (item) => CashFlowItem(
                  name: item['name']?.toString() ?? '',
                  amount: _toDouble(item['amount']),
                  type: item['type']?.toString() ?? 'financing',
                ),
              )
              .toList();
          cashFlowFromFinancing.value = _toDouble(financing['total']);

          netCashFlow.value = _toDouble(data['netCashFlow']);
          openingCashBalance.value = _toDouble(data['openingCashBalance']);
          closingCashBalance.value = _toDouble(data['closingCashBalance']);
          netCashFlowPercentage.value = _toDouble(
            data['netCashFlowPercentage'],
          );
        } else {
          hasError.value = true;
          errorMessage.value =
              responseData['message']?.toString() ?? 'Failed to load cash flow';
  ...
  Future<void> exportToPdf() async {
    try {
      ...
      final fileName =
          'cash_flow_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              _pdfSummaryItem(
                'Opening Balance',
                _formatAmount(openingCashBalance.value),
                PdfColors.indigo700,
              ),
              _pdfSummaryItem(
                'Closing Balance',
                _formatAmount(closingCashBalance.value),
                PdfColors.indigo700,
              ),
              _pdfSummaryItem(
                'Net Cash Flow',
                _formatAmount(netCashFlow.value),
          ...
          pw.Row(
            ...
            children: [
              ...
              pw.Text(
                '${netCashFlowPercentage.value.toStringAsFixed(1)}%',
                style: pw.TextStyle(
                  fontSize: 9,
                  color: netCashFlowPercentage.value >= 0
                      ? PdfColors.green700
                      : PdfColors.red700,
                ),
              ),
    ...
    try {
      ...
      for (final item in operatingItems) {
        ...
        _excelSetCell(
          ...
          item.amount,
          bgColor: bg,
          fontColor: item.amount >= 0 ? '2E7D32' : 'C62828',
        );
        row++;
      }
      _excelSetCell(
      ...
      _excelSetCell(
        ...
        bold: true,
        bgColor: 'E8EAF6',
        fontColor: cashFlowFromOperations.value >= 0 ? '2E7D32' : 'C62828',
      );
      ...
      _excelSetCell(
        ...
        bold: true,
        bgColor: 'E8EAF6',
        fontColor: cashFlowFromInvesting.value >= 0 ? '2E7D32' : 'C62828',
      );
      ...
      _excelSetCell(
        ...
        bold: true,
        bgColor: 'E8EAF6',
        fontColor: cashFlowFromFinancing.value >= 0 ? '2E7D32' : 'C62828',
      );
      ...
      _excelSetCell(
        ...
        netCashFlow.value,
        bold: true,
        fontColor: netCashFlow.value >= 0 ? '2E7D32' : 'C62828',
      );
      ...
      final dir = await getTemporaryDirectory();
      final fileName =
          'cash_flow_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.xlsx';
      final file = File('${dir.path}/$fileName');
