...
class JournalEntryController extends GetxController {
  ...
  // ✅ FIX: isLoading hata diya — ye sirf list screen ke liye hai.
  // Dialog apna _isSubmitting khud manage karta hai.
  // Agar isLoading yahan bhi set hota tha to dialog close hone ke baad
  // bhi screen pe loading spinner ghoomta rehta tha.
  Future<void> createJournalEntry({
    ...
    if (response.success) {
      ...
      // ✅ Delay reload so Navigator.pop(context) in dialog executes FIRST
      // before isLoading = true triggers — prevents "Posting..." stuck state
      Future.delayed(const Duration(milliseconds: 300), _resetAndReload);
    } else {
      ...
      // ✅ Throw so dialog's catch block resets _isSubmitting for retry
