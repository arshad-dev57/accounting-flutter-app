...
class JournalExportService {
    ...
    try {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (ctx) => [
            _pdfSummarySection(summary, branding.accent),
            pw.SizedBox(height: 8),
            pw.Text(
              'Total Entries Exported: ${entries.length}',
      ...
      if (kIsWeb) {
        ...
        if (result.type != ResultType.done) {
          AppSnackbar.error(
            Colors.red,
            'Error',
            'PDF khul nahi saka: ${result.message}',
          );
        }
      }
    } catch (e) {
  ...
  static pw.Widget _pdfSummarySection(
    Map<String, dynamic> summary,
    PdfColor accent,
  ) {
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          _pdfSummaryItem(
            'Total Debit',
            _formatAmount(summary['totalDebit'] ?? 0.0),
            PdfColors.green700,
          ),
          _pdfSummaryItem(
            'Total Credit',
            _formatAmount(summary['totalCredit'] ?? 0.0),
            PdfColors.red700,
          ),
          _pdfSummaryItem(
            'Difference',
      ...
      if (kIsWeb) {
        ...
        if (result.type != ResultType.done) {
          AppSnackbar.error(
            Colors.red,
            'Error',
            'Image khul nahi saka: ${result.message}',
          );
        }
      }
    } catch (e) {
      ...
      if (kIsWeb) {
        ...
        if (result.type != ResultType.done) {
          AppSnackbar.error(
            Colors.red,
            'Error',
            'Excel khul nahi saka: ${result.message}',
          );
        }
      }
    } catch (e) {
